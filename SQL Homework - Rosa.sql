...
SELECT actor_names.last_name, actor_names.last_name_count
FROM (
...
WHERE actor_names.last_name_count > 1;
...
-- SELECT * FROM payment WHERE payment_date BETWEEN '2005-08-01' AND '2005-08-31';

SELECT staff.staff_id, first_name, last_name, SUM(amount) AS TotalAmount
...
SELECT customer.email, address.city_id, city.country_id, country.country
FROM customer 
...
CREATE VIEW rev_by_category AS
	SELECT category.name, SUM(payment.amount) AS revenue
	FROM category
		JOIN film_category ON category.category_id = film_category.category_id
		JOIN inventory ON film_category.film_id = inventory.film_id
...
SELECT * FROM rev_by_category;

-- 8c. You find that you no longer need the view top_five_genres. Write a query to delete it.

DROP VIEW rev_by_category;
